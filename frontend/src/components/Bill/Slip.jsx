  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      for (const billId of filteredBillIds) {
        await axios.put(`http://localhost:5012/bill/${billId}`, {
          status: "pending",
        });
      }

      const response = await axios.post("http://localhost:5012/slips", {
        User_ID: id,
        Value: total,
        imagePath: imageUrl,
      ...
      alert(`Failed to create slip. Please check the console for details.`);
    }
  };
