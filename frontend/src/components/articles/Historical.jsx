...
function Historical({ searchQuery }) {
  ...
  useEffect(() => {
    const fetchArticles = async () => {
      try {
        const response = await axios.get(
          `http://localhost:5012/api/articles?category=Historical%20Sites&title=${searchQuery}`
        );
        setFound(true);
  ...
  return (
    <div className="flex flex-col items-center min-h-screen bg-gray-100">
      <h1 className="text-3xl font-bold mb-8">Historical Sites</h1>
      {loading ? (
        <p>Loading...</p>
      ) : found ? (
