    ...
    const fetchInitialViewCounts = async () => {
      try {
        ...
        for (const article of articles) {
          ...
          initialViewCounts[article._id] = response.data.article.views;
        }
        setViewCounts(initialViewCounts);
      } catch (error) {
        console.error("Error fetching initial view counts:", error);
      }
    };
...
function CulExp({ searchQuery }) {
  ...
  useEffect(() => {
    const fetchArticles = async () => {
      try {
        const response = await axios.get(
          `http://localhost:5555/api/articles?category=Cultural%20Experience&title=${searchQuery}`
        );
        setFound(true);
  ...
  return (
    <div className="flex flex-col items-center min-h-screen bg-gray-100">
      <h1 className="text-3xl font-bold mb-8">Cultural Experience</h1>
      {loading ? (
        <p>Loading...</p>
      ) : found ? (
